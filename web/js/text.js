function prepareGamebook()
{
	...
	var $epHeader = $container.find("h2:contains('Р•РїРёР·РѕРґРё')");
	...
	var $help = $('<div class="notice" style="margin: 15em auto 45em">\
		<hr/>\
		<p>РќРµР°РєС‚РёРІРЅРёС‚Рµ РµРїРёР·РѕРґРё Р±СЏС…Р° СЃРєСЂРёС‚Рё. Р©Рµ СЃРµ РїРѕРєР°Р¶Р°С‚ РІ С…РѕРґР° РЅР° РёРіСЂР°С‚Р°.</p>\
		<div class="standalone"><a href="#l-1" class="ep-all">РџРѕРєР°Р·РІР°РЅРµ РЅР° РІСЃРёС‡РєРё РµРїРёР·РѕРґРё</a></div>\
		<div class="standalone">РћС‚РёРІР°РЅРµ РЅР° РµРїРёР·РѕРґ <input type="text" name="ep-goto" size="4"></div>\
		<hr/>\
		</div>');
	...
	$container.on("click", "a", function() {
		...
		if (match) {
			...
			var linkText = driver.lastEp == 0 ? 'РќР°Р·Р°Рґ' : 'РќР°Р·Р°Рґ РєСЉРј РµРїРёР·РѕРґР°';
	...
	var enhanceChanceTable = function(idx, table) {
		...
		var $handle = $('<button style="display: block; margin: .5em auto">РЎР»СѓС‡Р°Р№РЅРѕ С‡РёСЃР»Рѕ</button>').click(function() {
	...
	function replaceInputPlaceholders(str, name) {
		return str
			// вЂ¦ x3
			.replace(/вЂ¦вЂ¦вЂ¦/g, '<textarea style="width: 99%; height: 5em"></textarea>')
			// (вЂ¦=radio 1 / radio 2 / radio 3)
			.replace(/\(вЂ¦=([^)]+)\)/, function(m0, m1) {
				var elems = [];
			...
			// [вЂ¦=checkbox 1 / checkbox 2 / checkbox 3]
			.replace(/\[вЂ¦=([^\]]+)\]/, function(m0, m1) {
				var elems = [];
				m1.split(" / ").forEach(function(value, i) {
			...
			// вЂ¦(=initial value)
			.replace(/вЂ¦\(=([^)]+)\)/g, '<input type="text" style="width: 98%" value="$1">')
			// вЂ¦(placeholder value)
			.replace(/вЂ¦\(([^)]+)\)/g, '<input type="text" style="width: 99%" placeholder="$1">')
			.replace(/вЂ¦/g, '<input type="text" style="width: 99%">');
	}
	...
	$inputContainer
		.wrap('<form></form>')
		.append('<div style="text-align:right"><input type="reset" value="Р—Р°РЅСѓР»СЏРІР°РЅРµ РЅР° Р·Р°РїРёСЃРёС‚Рµ"></div>')
	...
	$(".js-gamebook-board").each(function() {
		...
		.attr("title", "Р©СЂР°РєРЅРµС‚Рµ, Р·Р° РґР° СЃР»РѕР¶РёС‚Рµ РёР»Рё РїСЂРµРјРµСЃС‚РёС‚Рµ РїРёРѕРЅРєР°С‚Р°")
