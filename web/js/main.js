function showErrorToUser(response) {
	var message = "Р’СЉР·РЅРёРєРЅР°Р»Р° Рµ РіСЂРµС€РєР°: " + response.statusText + " (" + response.status + ")";
	switch (response.status) {
		case 401: message = "РќСЏРјР°С‚Рµ РЅРµРѕР±С…РѕРґРёРјРёС‚Рµ РїСЂР°РІР° Р·Р° С‚РѕРІР° РґРµР№СЃС‚РІРёРµ."; break;
	}
	alert(message);
}
	...
	$("body")
		.delegate("a.action-new", "click", function(){
			if (user.canTakeAction(this)) {
				...
				alert("РќСЏРјР°С‚Рµ РЅРµРѕР±С…РѕРґРёРјРёС‚Рµ РїСЂР°РІР° Р·Р° С‚РѕРІР° РґРµР№СЃС‚РІРёРµ.");
			}
		...
		.delegate("form.delete-form", "submit", function(){
			if (user.canTakeAction(this)) {
				if (confirm("РќР°РёСЃС‚РёРЅР° Р»Рё РґР° СЃРµ РёР·С‚СЂРёРµ? Р’СЂСЉС‰Р°РЅРµ РЅР°Р·Р°Рґ РЅСЏРјР°.")) {
		...
		$.post(_GLOBALS.scriptname + "user-special-texts", {texts: textIds}, function(response){
			$texts.each(function(){
				...
				if (typeof response.favorities[id] == "number") {
					var newText = "РџСЂРµРјР°С…РІР°РЅРµ РѕС‚ РР·Р±СЂР°РЅРё";
...
(function(){
	...
	var togglerHtml = '<div>\
		<a href="#hide-sidebar" class="sidebar-toggle hide-toggle fa-stack" title="РЎРєСЂРёРІР°РЅРµ РЅР° РјРµРЅСЋС‚Рѕ">\
			<span class="fa fa-bars fa-stack-1x"></span>\
			<span class="fa fa-ban fa-stack-2x text-warning"></span>\
		</a>\
		<a href="#'+$sidebar.attr('id')+'" class="sidebar-toggle show-toggle hide fa fa-bars" title="РџРѕРєР°Р·РІР°РЅРµ РЅР° РјРµРЅСЋС‚Рѕ"></a>\
		</div>';
...
function saveRating(form) {
	if ( ! user.isAuthenticated()) {
		alert("РЎР°РјРѕ СЂРµРіРёСЃС‚СЂРёСЂР°РЅРё РїРѕС‚СЂРµР±РёС‚РµР»Рё РјРѕРіР°С‚ РґР° РґР°РІР°С‚ РѕС†РµРЅРєРё.");
